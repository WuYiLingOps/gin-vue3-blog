...
> 全新单栏卡片流：作者介绍渐变卡、个人信息卡、MBTI 性格卡、技能图标跑马灯、成长轨迹时间线、追求卡（轮换词动效）、地理位置地图、自我介绍、文章统计、相册与评论区，全部内容支持后台可视化配置。
...
- **关于我页面** - 单栏卡片流全面重构：作者介绍渐变卡、个人信息卡、MBTI 性格卡（人格形象图）、技能品牌图标双行跑马灯（simple-icons）、成长轨迹时间线、追求卡（轮换词动效）、地理位置地图（高德瓦片实时渲染 + 悬浮缩放动效），全部板块内容支持管理后台可视化配置
- **主题切换** - 支持亮色/暗色主题
...
- **修订版本管理** - 编辑超级管理员文章时自动创建修订版本，记录每次修改的完整历史
  - 支持查看文章的所有修订版本列表（修订时间、修改说明、修改人）
  - 支持对比任意两个版本的差异（标题、摘要、内容、分类、标签）
  - 采用 diff-match-patch 算法实现精确的文本差异计算
  - 行内高亮展示：删除内容显示红色背景+删除线，新增内容显示绿色背景
  - 只显示修改部分及上下文（前后各100字符），类似 Git diff 效果
  - 支持填写修改说明，方便追溯修改原因
  - 我的修订版本页面：查看当前用户对其他超级管理员文章的所有修订记录
- **文章协作者** - 管理员编辑他人文章时自动成为协作者，每篇文章最多1个协作者
  - 文章列表页显示第一作者和协作者头像、昵称
  - 文章详情页同样展示协作者信息
  - 自动添加规则：编辑者 ≠ 文章作者 且 文章从未有过协作者
  - 移除权限：
    - 协作者是超级管理员 → 只有超级管理员自己可以移除（站长权威）
    - 协作者是普通管理员 → 文章作者或超级管理员可以移除
  - 移除后不会被自动重新添加（采用软删除机制记录历史）
- 文章分类和标签
- 文章置顶和草稿
- 点赞和浏览统计
- **文章导出** - 支持两种导出格式：
...
- **用户管理**（用户列表、状态管理、限制用户注册功能、角色管理）
  - 删除用户时事务内统一清理关联数据（修订记录、说说、点赞、令牌等），其名下文章自动转移给超级管理员，浏览统计置空保留
...
- **关于我管理** - 标签页布局：正文 Markdown 独立编辑卡 + 8 个配置板块分页（作者介绍 / 个人信息 / MBTI 性格 / 地理位置 / 打字词轮播 / 技能 / 成长轨迹 / 追求卡），长表单滚动时悬浮保存条保持可见
- IP 访问控制（黑名单/白名单统一管理）
...
- **订阅者管理**（管理后台 - 仅超级管理员可见）
  ...
  - 显示活跃订阅者数量和累积订阅总数
- **推送历史记录**（管理后台）
  - 查看所有推送历史（文章标题、推送时间、成功/失败数）
  - 查看单次推送的详细结果（每个订阅者的发送状态和错误信息）
  - 推送统计数据（总推送次数、成功率、失败率）
  - 支持删除历史记录
- **技术实现**
...
- **技术实现**
  ...
  - HTTP 缓存优化：支持 ETag 和 Last-Modified 响应头，客户端可使用 304 Not Modified 减少带宽
  - 自动过滤草稿和私密内容
  - RSS 是无状态协议，无法统计订阅人数
...
- `DELETE /api/posts/:id/collaborators/:userId` - 移除文章协作者（需认证，仅文章作者或超级管理员）

## 9.2.1 文章修订版本相关

- `GET /api/admin/post-revisions/pending` - 获取待审批修订列表（超级管理员）
- `GET /api/admin/post-revisions/pending/count` - 获取待审批修订数量（超级管理员）
- `GET /api/admin/post-revisions/:id` - 获取修订详情（需认证）
- `GET /api/admin/post-revisions/:id/diff` - 获取修订对比（需认证）
- `POST /api/admin/post-revisions/:id/approve` - 审批通过（超级管理员）
- `POST /api/admin/post-revisions/:id/reject` - 审批拒绝（超级管理员）
- `POST /api/admin/post-revisions/:id/withdraw` - 撤回修订（创建者）
- `GET /api/admin/post-revisions/my` - 获取当前用户的修订记录（需认证）
