<template>
  <div class="ip-blacklist-manage-page">
    <h1 class="page-title">IP 黑名单管理</h1>
    ...
    <n-modal v-model:show="showCheckResultModal">
      ...
      >
        <div v-if="checkResult">
          <n-alert
            :type="checkResult.banned ? 'error' : 'success'"
            style="margin-bottom: 16px"
          >
            <template #header>
              {{ checkResult.banned ? '该 IP 已被封禁' : '该 IP 未被封禁' }}
            </template>
          </n-alert>
          <div v-if="checkResult.banned && checkResult.info" class="check-result-info">
            <n-descriptions :column="1" bordered>
              <n-descriptions-item label="IP 地址">
                {{ checkResult.info.ip }}
              </n-descriptions-item>
              <n-descriptions-item label="封禁类型">
                <n-tag :type="checkResult.info.ban_type === 1 ? 'warning' : 'error'">
                  {{ checkResult.info.ban_type === 1 ? '自动封禁' : '手动封禁' }}
                </n-tag>
              </n-descriptions-item>
              ...
              <n-descriptions-item label="过期时间">
                {{ checkResult.info.expire_at ? formatDate(checkResult.info.expire_at) : '永久封禁' }}
              </n-descriptions-item>
        ...
        <template #footer>
          <n-space justify="end">
            ...
            <n-button
              v-if="checkResult?.banned && checkResult.info"
              type="error"
...
async function handleCheck() {
  try {
    ...
    checking.value = true
    const res = await checkIPStatus(checkForm.value.ip)
    if (res.data) {
...
@media (max-width: 768px) {
  ...
  .ip-blacklist-manage-page :deep(.n-data-table) {
    font-size: 13px;
  }
}
</style>
