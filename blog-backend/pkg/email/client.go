...
// SendFriendApplyNotification 发送友链申请邮件（管理员）
func (c *Client) SendFriendApplyNotification(to, siteName, applicantName, friendLinkName, friendLinkURL string) error {
	if !c.rateLimiter.Allow(to) {
		return fmt.Errorf("发送频率过高，请稍后再试")
	}
	...
	subject := fmt.Sprintf("【%s】新的友链申请", siteName)

	htmlApplicantName := template.HTMLEscapeString(applicantName)
	htmlFriendLinkName := template.HTMLEscapeString(friendLinkName)
	htmlFriendLinkURL := template.HTMLEscapeString(friendLinkURL)
...
<head>
    ...
    <style>
        body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; background-color: #f5f5f5; margin: 0; padding: 0; }
        .container { max-width: 600px; margin: 0 auto; padding: 20px; }
        .header { background: linear-gradient(135deg, #a855f7 0%%, #c084fc 100%%); color: white; padding: 40px 30px; text-align: center; border-radius: 12px 12px 0 0; box-shadow: 0 4px 12px rgba(168, 85, 247, 0.2); }
        .header h2 { margin: 0; font-size: 28px; font-weight: 700; }
        .content { background: white; padding: 40px 30px; border-radius: 0 0 12px 12px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1); }
        .content p { margin: 16px 0; font-size: 15px; color: #475569; }
        .content strong { color: #a855f7; font-weight: 600; }
        .info-box { background: #faf5ff; border-left: 4px solid #a855f7; padding: 16px; margin: 24px 0; border-radius: 4px; }
        .info-box p { margin: 8px 0; color: #475569; }
        .btn { display: inline-block; background: linear-gradient(135deg, #a855f7 0%%, #c084fc 100%%); color: white; padding: 14px 32px; text-decoration: none; border-radius: 8px; font-weight: 600; box-shadow: 0 4px 12px rgba(168, 85, 247, 0.3); }
        .btn:hover { box-shadow: 0 6px 16px rgba(168, 85, 247, 0.4); }
        .footer { margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb; color: #94a3b8; font-size: 13px; text-align: center; }
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="info-box">
                ...
                <p><strong>网站名称：</strong>%s</p>
                <p><strong>网站地址：</strong>%s</p>
            </div>
            <div style="text-align: center;">
                <a href="%s" class="btn">查看申请</a>
            </div>
        </div>
        <div class="footer">
            <p>这是来自 <strong>%s</strong> 的友链申请通知。</p>
            <p>此邮件由系统自动发送，请勿直接回复</p>
        </div>
    </div>
</body>
</html>
`, htmlApplicantName, htmlFriendLinkName, htmlFriendLinkURL, friendLinkURL, siteName)

	textBody := fmt.Sprintf(`友链申请通知
...
---
这是来自 %s 的友链申请通知。
此邮件由系统自动发送，请勿直接回复`, applicantName, friendLinkName, friendLinkURL, friendLinkURL, siteName)
